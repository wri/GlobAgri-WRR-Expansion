...
EE_COLLECTION = 'users/globagriwrr001'
GS_FOLDER = 'SPAM'
GS_BUCKET = 'gs://globagri-upload-bucket/HYDE/'
...
ASC_FILENAME = '{}2010AD.asc'
TIF_FILENAME = '{}2010AD.tif'
VARIABLES = ['iso','maxln']#['conv_rangeland','pasture','rangeland','conv_rangeland']


BANDS = ['iso','maxln']#['grazing','pasture','rangeland','conv_rangeland']#['landlake']#['cropland']#
...
def upload_asset(varname,NDV,DATA_DIR=DATA_DIR,EE_COLLECTION=EE_COLLECTION,GS_BUCKET=GS_BUCKET,VARIABLES=VARIABLES):
    ...
    TEMP_FILE_NAME = '{}_2010AD_reprj.tif'.format(varname)
...
for i,varname in enumerate(VARIABLES):
    ...
    #for band in BANDS:
    convert_ascii_to_tif(ASC_FILENAME.format(varname), TIF_FILENAME.format(varname),varname)
    #Combine multiple geotiffs to one geotiff with multiple bands
    #combine_tiffs(NDV,varname)

    in_file = DATA_DIR+'{}2010AD.tif'.format(varname)
    out_file = DATA_DIR+'{}_2010AD_reprj.tif'.format(varname)
    ...
    raster_clip(reference_file, in_file, out_file, srcnodata = ndv, dstnodata=ndv, resampling_method=resampling_method,out_format=data_type)
    #Uplaod asset
